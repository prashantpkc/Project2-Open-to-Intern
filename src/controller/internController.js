...
module.exports.createIntern = async (req, res)=> {
  try {
    ...
    if (!isValid(mobile))
      return res
        .status(400)
        .send({ status: false, msg: "Mobile number is required field " });
    if (!isValid(collegeName))
      return res
        .status(400)
        .send({ status: false, msg: "collegeName is required field" });
    if (!isValidName(name))
      return res
        .status(400)
        .send({ status: false, msg: "Pls Enter Valid name" });
    if (!isValidEmail(email))
      return res
        .status(400)
        .send({ status: false, msg: "Pls Enter Valid Email" });
    if (!isValidMobile(mobile))
      return res
        .status(400)
        .send({ status: false, msg: "Pls Enter Valid mobile Number" });
    ...
    if (checkUnique)
      return res
        ...
        .send({
          status: false,
          msg: "this email is already registered Pls Use Unique Email",
        });
    ...
    if (!collegeId)
      return res
        ...
        .send({
          status: false,
          msg: " No college found ,Pls Enter Valid College Name",
        });
