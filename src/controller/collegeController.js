...
let { isValid, isValidName, isValidUrl } = require("../Validator/validation");

module.exports.createCollegeData = async function (req, res) {
  try {
    ...
    if (!isValid(name))
      return res
        .status(400)
        .send({ status: false, msg: "name is required field, please enter" });

    if (!isValidName(fullName))
      return res
        .status(400)
        .send({ status: false, msg: "Pls Enter Valid fullName " });
    if (!isValidUrl(logoLink))
      return res
        .status(400)
        .send({ status: false, msg: "Pls Enter Valid logoLink " });
    if (!isValidName(name))
      return res
        .status(400)
        .send({
          status: false,
          msg: "please enter valid name(between A-Z or a-z)",
        });

    let checkNameNotDeleted = await collegeModel.findOne({
      name: name,
      isDeleted: false,
    });
    if (checkNameNotDeleted)
      return res
        .status(400)
...
module.exports.getDetails = async function (req, res) {
  try {
    ...
    if (Object.keys(query).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "Don't Left Query Params Empty" });
    const query1 = query.collegeName.trim().toLowerCase();

    if (!isValid(query1))
      return res
        .status(400)
        .send({ status: false, msg: "Dont Left The Query Tag Value Empty" });
    let getCollegeDetails = await collegeModel
      .findOne(
        { name: query1 },
        { name: 1, fullName: 1, logoLink: 1, isDeleted: 1 }
      )
      .lean();
    ...
    if (!getCollegeDetails)
      return res
        .status(404)
        .send({ status: true, msg: " This College Name Doesn't Exists" });

    let SelectCollegeId = getCollegeDetails._id;

    let getInternDetails = await internModel
      .find({ collegeId: SelectCollegeId })
      .select({ name: 1, email: 1, mobile: 1 });
    if (getInternDetails.length == 0)
